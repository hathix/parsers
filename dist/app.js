...
_.mixin({
    ...
    'indicesOf': function indicesOf(list, needle) {
 ...
 */
function generateTable2d(rowValues, rowLabelFn, columnValues, columnLabelFn, cellFn) {
    var table = {};
    _.each(rowValues, function (rowValue) {
        var currentRow = table[rowLabelFn(rowValue)] = {};
        _.each(columnValues, function (columnValue) {
            currentRow[columnLabelFn(columnValue)] = cellFn(rowValue, columnValue);
        });
    });
...
function followOfSymbol(symbol) {
    if (symbol === START) {
        ...
        var validRules = _.filter(PRODUCTION_RULES, function (rule) {
            return _.includes(_.slice(rule.right, 0, rule.right.length - 1), symbol);
        });
...
var parseTable = generateTable2d(nonterminalData, function (nonterminal) {
    ...
    var validRules = _.filter(nonterminalRules, function (rule) {
        return _.includes(rule.first, terminal) || _.includes(rule.first, EMPTY) && _.includes(nonterminal.follow, terminal);
    });
...
function parse(rawInputList) {
     ...
     * recursively.
     */
    var parseHelper = function parseHelper(_x2, _x3, _x4) {
        var _again2 = true;
        ...
        _function2: while (_again2) {
            var input = _x2,
                stack = _x3,
                tree = _x4;
            topStackSymbol = topInputSymbol = rule = undefined;
            _again2 = false;
            ...
            if (stack.isEmpty()) {
                ...
                if (topStackSymbol === topInputSymbol) {
                    ...
                    _x2 = input;
                    _x3 = stack.pop().pushAll(rule.right);
                    _x4 =
